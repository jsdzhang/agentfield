...
from brain_sdk.logger import log_debug, log_warn
...
class AgentWorkflow:
    """Workflow helper that keeps local execution metadata in sync with Brain."""
    ...
    async def fire_and_forget_update(self, payload: Dict[str, Any]) -> None:
        """Send workflow update to Brain when a client is available."""

        client = getattr(self.agent, "client", None)
        base_url = getattr(self.agent, "brain_server", None)
